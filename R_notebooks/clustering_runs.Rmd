...
- plot things based on success/failure

- try Nic's envelope graphs comparing lexicase to tourney

```{r}
ggplot(all_data, aes(x=all_data$generation)) + 
  geom_smooth(aes(y=all_data$error.diversity, color=" Diversity")) + 
  labs(y="% of population", color="")
```

```{r}
ggplot(all_data, aes(x=all_data$generation)) + 
  geom_smooth(aes(y=all_data$cluster.count, color=" Diversity")) + 
  labs(y="% of population", color="")
```
...
rswn_div_plot <- ggplot(all_rswn_data, aes(x=generation, y=error.diversity,
  ...
  scale_color_discrete(breaks=c("lexicase", "tourney", "ifs"))
...
rswn_lex_success_plot <- ggplot(success_counts, aes(x=generation, y=num.successes, color=treatment)) + geom_line() + ylim(c(0,100)) + scale_color_discrete(breaks=c("lexicase", "tourney", "ifs"))
