...
This is run once to transform the data file into an errors-only file.
...
```{r}
errors1 <- read.csv("data/RSWN/lexicase/errors_data1.csv")
```
...
```{r}
errors6 <- read.csv("data/RSWN/lexicase/errors_data6.csv")
```
...
```{r}
data_frame6 = make_frame_from_errors_file("data/RSWN/lexicase/errors6.csv", 6, "replace-space-with-newline", "lexicase", 20, elitize_generation_data)
```