...
And lastly the iterated fitness sharing (IFS) data:

```{r}
ifs_file_list = list.files(path="../data/RSWN/ifs/clustering/", 
                               pattern="*.csv")
ifs_frames = lapply(paste("../data/RSWN/ifs/clustering/", 
                              ifs_file_list, sep=""), read.csv)

padded_ifs_frames = lapply(ifs_frames, pad_frame)

ifs_data = do.call(rbind, padded_ifs_frames)
```

Let's combine all that into a single great big data frame:

```{r}
rswn_data <- rbind(lexicase_data, tourney_data, ifs_data)
```
...
ggplot(rswn_data, aes(x=generation, y=error.diversity,
                      ...
                      fill=interaction(succeeded, treatment), alpha=0.05)) +
  geom_point(alpha=0.02) +
  stat_summary(fun.data="median_hilow", conf.int=0.5, geom="ribbon", alpha=0.05) +
#  geom_smooth() +
...
```{r}
wilcox_ci <- function (data, gen, first_treatment, second_treatment, value) {
...
lex_tourney_cis <- melt(sapply(seq(0, 299), function (gen) wilcox_ci(rswn_data, gen, 'lexicase', 'tourney', 'cluster.count')))

ggplot(lex_tourney_cis, aes(x=Var2, y=value, group=Var1)) + geom_line()
```
